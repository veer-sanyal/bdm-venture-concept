...
- **AI-native commissioning for liquid-cooled AI data halls: 11.50 from 6 clean judges, advances.** The first run gave 11.6, but 4 of its 5 judges had read METHOD.md. The clean re-judge confirmed the advance ([re-judge](research/2026-09-25-rejudge/RESULT.md), [audit](research/2026-09-25-loop2/contamination-audit.md)). It is the strongest candidate on clean scores, 0.57 above the bar.
  - Strongest version (5 of 6 clean judges): an independent commissioning agent hired by the owner (one judge argued for the general contractor's side instead) for neoclouds, second-tier developers and converted bitcoin miners. It sells live step-by-step grading of the integrated test, with a signed report within 24 hours. Price is per MW plus a bonus for on-schedule handover.
...
- **Truck dealer warranty claims: 9.0 from 3 clean judges, stops.** At 1% of $5.41B the US truck market is about $54M a year. Dealer management systems own the claim rail, and Daimler's warranty system already gives dealers free AI coding help. Loop 1's dealer-warranty idea (RV and marine) had already been dropped.
- **Standings (bar 10.93):** commissioning **11.50** (clean, n=6) advances. Credit disputes **11.00** (clean re-judge, n=5; 11.2 unaudited in loop 1) advances by 0.07, which is a tie within noise. Arc flash 10.6, LTC Medicaid 10.0, truck warranty 9.0 and Assay 9.0 stop.
  - Credit disputes, clean judges' strongest version: debt buyers and large collectors first, an e-OSCAR API middleware vendor keeping the litigation-ready investigation file of record, sold through FCRA defense firms or collection software.
  - Killers: deleting is nearly free; the complaint surge mostly hits the three bureaus (about 5.1M of 5.8M); CFPB portal friction; Provana/Sonnet and Bridgeforce on both sides.
  - Test: ask 10 compliance heads for cost per dispute, delete rate and FCRA suits, and offer a paid pilot at $1.50 to $2.
- **METHOD change adopted 2026-09-25 (Veer):** the shaper, judges and team fit may not read local project files. The shaper gets the merged ideas pasted in, and AGENTS.md exempts METHOD agents from its reading list. The loop 2 audit found the shaper, 9 of 17 judges and the team-fit agent had read METHOD, STATE or both. Loop 1 could not be audited, because its transcripts are gone. Every agent transcript must now pass `tools/audit_reads.py` before its report or scores are used, and a flagged judge is excluded and replaced. The nine flagged loop 2 judge rows are marked `excluded` in `scores.csv`.
- Decision 4 below (the concept to name for the 9/27 prelim): on clean scores, commissioning (11.50) leads credit disputes (11.00).
