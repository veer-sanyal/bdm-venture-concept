...
- **Decisions (Veer, 2026-09-24 evening):**
  1. **Leaner loop adopted** and written into METHOD: one shaper per idea, a stored control bank, judges added one at a time until the mean is outside 2s/√n of the bar (max 5), and one reshape pass for candidates within 1.0 below the bar. `tools/scores.py` now marks what each paragraph needs next.
  1a. **The control bank keeps growing** (Veer, same evening). Every loop draws 3 new AI-related 2026 YC companies at random and gives each 3 judges. The bar, its SE (0.37 now), the judge noise and the judge bands recompute from the whole bank. The script flags drift between a round's controls and the rest, and marks candidates closer to the bar than the combined uncertainty as close calls (all three loop 1 candidates are). The 7 single-judge controls are not topped up; new controls are the better use of judges.
